...
object SequenceGraphUtils {
  //TODO: add gene-id as well to distinguish sub-graphs from different genes
  case class AminoAcid(id: Int, aa: Char)
  ...
  /**
    ...
    */
  def msa2SequenceGraph(msa_file: File): (SequenceGraph, SequencePaths) = {
    ...
    @tailrec def _msa2SequenceGraph(seqname: String,
      ...
      def setSequence(): List[AminoAcid] =
        seq.toString().toList.zipWithIndex.filter(_._1 != '-').map(x => new AminoAcid(x._2, x._1))
